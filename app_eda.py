...
class EDA:
    def __init__(self):
        ...
        # — 전처리: '-' 값을 0으로 치환하고 숫자형 변환 —
        pop_df[cols] = pop_df[cols].replace('-', 0)
        pop_df[cols] = (pop_df[cols]
        ...
        with tabs[1]:
            ...
            avg_net = net.mean()
            years_to_2035 = 2035 - int(recent['연도'].iloc[-1])
            pred = int(recent['인구'].iloc[-1] + avg_net * years_to_2035)
            ax.scatter([2035], [pred], color='red')
            ax.text(2035, pred, f"{pred:,}")
            st.pyplot(fig)
        ...
        with tabs[2]:
            ...
            prev5 = latest - 5
            df5 = (pop_df[(pop_df['연도'].isin([prev5, latest])) & (pop_df['지역'] != "전국")]
